...
with st.sidebar:
    ...
    st.markdown(
        """
        <small>If running on Streamlit Cloud, you **must** use a public API URL (like ngrok or a deployed server). Localhost will not work.</small>
        """, 
    ...
    api_url = st.text_input(
        "RAGify API URL", 
        value="", 
        placeholder="https://your-public-server.com/api/v1/rag/query"
    )
...
if prompt := st.chat_input("Ask a question about your documents..."):
    ...
    if not api_url or not api_key:
        st.error("Please provide both an API URL and API Key in the sidebar.")
        st.stop()
    ...
    with st.chat_message("assistant"):
        ...
        try:
            ...
            st.error("Connection Refused! The Streamlit app could not reach your API URL. Make sure you are using a public URL (like ngrok) and not localhost.")
            full_response = "Connection failed."
